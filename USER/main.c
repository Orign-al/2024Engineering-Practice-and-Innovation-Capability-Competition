...
#define POSITION_DEADBAND 18  // 死区值，单位取决于编码器的单位
...
void InitPIDControllers(void) {
	...
	if(mini_pos_corr == 0)
	{
		...
		// 初始化位置环PID控制器
		PID_Init(&position_pid_controllers[0], 35.0f, 2.8f, 0.0f, 1000.0f);
		PID_Init(&position_pid_controllers[1], 35.0f, 2.5f, 0.0f, 1000.0f);
		PID_Init(&position_pid_controllers[2], 35.0f, 2.8f, 0.0f, 1000.0f);
		PID_Init(&position_pid_controllers[3], 35.0f, 2.5f, 0.0f, 1000.0f);

        if(sign_bazhi==0)
		{
            PID_Init(&position_pid_controllers[4], 2.0f, 0.08f, 1000.0f, 1000.0f);
            PID_Init(&position_pid_controllers[5], 2.0f, 0.08f, 1000.0f, 1000.0f);
        }
        else if (sign_bazhi==1)
		{
            PID_Init(&position_pid_controllers[4], 4.0f, 1.7f, 0.0f, 1000.0f);
            PID_Init(&position_pid_controllers[5], 4.0f, 1.7f, 0.0f, 1000.0f);
        }

		// 初始化速度环PID控制器
		PID_Init(&speed_pid_controllers[0], 1.8f, 2.5f, 0.012f, 100.0f);
		PID_Init(&speed_pid_controllers[1], 1.8f, 2.5f, 0.012f, 100.0f);
		PID_Init(&speed_pid_controllers[2], 1.8f, 2.5f, 0.012f, 100.0f);
		...
		for (int i = 0; i < MOTOR_COUNT; i++) {
			...
			PID_SetOutputLimits(&speed_pid_controllers[i], -2000, 2000);
	...
	if(mini_pos_corr)
	{
		...
		PID_Init(&angle_pid_controllers, 4.0f,5.0f, 0.001f, 360.0f);  // 角度环参数
        ...
        if(sign_bazhi==0)
		{
            PID_Init(&position_pid_controllers[4], 18.0f, 0.7f, 0.0f, 1000.0f);
            PID_Init(&position_pid_controllers[5], 18.0f, 0.7f, 0.0f, 1000.0f);
        }
        else if (sign_bazhi==1)
		{
            PID_Init(&position_pid_controllers[4], 40.0f, 3.7f, 0.0f, 1000.0f);
            PID_Init(&position_pid_controllers[5], 40.0f, 4.3f, 0.0f, 1000.0f);
        }
		...
		for (int i = 0; i < MOTOR_COUNT; i++) {
            if(sign_bazhi==0)
			{
    			PID_SetOutputLimits(&position_pid_controllers[i], -25, 25);
            }
            else if (sign_bazhi==1)
			{
    			PID_SetOutputLimits(&position_pid_controllers[i], -200, 200);
			}
			PID_SetOutputLimits(&speed_pid_controllers[i], -2000, 2000);
			PID_SetOutputLimits(&current_pid_controllers[i], -6000, 6000);
		}
    ...
    if(mini_pos_corr==2)
	{
		...
		// 初始化位置环PID控制器
		PID_Init(&position_pid_controllers[0], 20.0f, 20.8f, 0.0f, 1000.0f);
		PID_Init(&position_pid_controllers[1], 20.0f, 20.8f, 0.0f, 1000.0f);
		PID_Init(&position_pid_controllers[2], 20.0f, 20.8f, 0.0f, 1000.0f);
		PID_Init(&position_pid_controllers[3], 20.0f, 20.8f, 0.0f, 1000.0f);
		...
		for (int i = 0; i < MOTOR_COUNT; i++) {
			...
			PID_SetOutputLimits(&current_pid_controllers[i], -10000, 10000);
		}
		// 角度环限幅只需对一个控制器进行设置
		PID_SetOutputLimits(&angle_pid_controllers, -360, 360);
...
void pi_data_process(char *rx_buffer ,int32 *x ,int32 *y, int *put_obj_fir , int *put_obj_sec, int *command) {
    ...
    else if(rx_buffer[0] == 'B' && rx_buffer[1] == 'Z' && rx_buffer[strlenmax-2] == 'Z' && rx_buffer[strlenmax -1] == 'B' ){
        sscanf(rx_buffer, "BZ%d+%dZB", put_obj_fir, put_obj_sec);
        ClearRxBuffer();        
    }
    else if(rx_buffer[0] == 'C' && rx_buffer[1] == 'Z' && rx_buffer[strlenmax-2] == 'Z' && rx_buffer[strlenmax -1] == 'C' ){
...
void TIM7_IRQHandler(void) {
    if (TIM_GetITStatus(TIM7, TIM_IT_Update) != RESET) {
        ...
        float camera_x_error = (float)x_move;  // 假设 x_move 是摄像头反馈的 X 轴误差
        float camera_y_error = (float)y_move;  // 假设 y_move 是摄像头反馈的 Y 轴误差

        // 计算基于摄像头反馈的位置校正
        float position_correction_x = -PID_Compute(&position_pid_controllers[4], 0, camera_x_error);
        float position_correction_y = -PID_Compute(&position_pid_controllers[5], 0, camera_y_error);
        ...
        // 位置环PID计算，基于电机编码器反馈和目标位置，应用Yaw校正和摄像头位置校正
        float position_pid_output1 = PID_Compute(&position_pid_controllers[0], Move_moters_speed.moter_speed_1 - yaw_correction + position_correction_x - position_correction_y, motors[0].revolutions);
        float position_pid_output2 = PID_Compute(&position_pid_controllers[1], Move_moters_speed.moter_speed_2 - yaw_correction + position_correction_x + position_correction_y, motors[1].revolutions);
        float position_pid_output3 = PID_Compute(&position_pid_controllers[2], Move_moters_speed.moter_speed_3 - yaw_correction - position_correction_x - position_correction_y, motors[2].revolutions);
        float position_pid_output4 = PID_Compute(&position_pid_controllers[3], Move_moters_speed.moter_speed_4 - yaw_correction - position_correction_x + position_correction_y, motors[3].revolutions);
...
void move_relative(float dx, float dy, float dz, float dyaw) {
    Move_moters_speed.moter_speed_1 += dx - dy - dz;
    Move_moters_speed.moter_speed_2 += dx + dy + dz;
    Move_moters_speed.moter_speed_3 += -dx - dy + dz;
    Move_moters_speed.moter_speed_4 += -dx + dy + dz;
    target_yaw += dyaw;
...
int main(void) {
    ...
    //行走、初始姿态
    initial_pos();
	InitializeMotorFineTuneControllers();
    ...
    while (1) {
        ...
        // #define POSITION_DEADBAND 1  // 死区值，单位取决于编码器的单位
        // mini_pos_corr = 2;
        // InitPIDControllers(); 
       ...
       while(1){
        ...
        target_yaw = 47;
        set_target_position(-190, 30, 0, 0);
        right = 1;
        left = 0;
        up = 0;
        down = 0;
		...
		while( pi_command != 9991 || pi_command != 9992 || pi_command != 9993){
			...
			if(x_pi<=1000&&y_pi<=1000 ){
				...
				target_x = -190 + x_move ;
				target_y = 30 + y_move ;
				// move_relative(x_move ,y_move ,0,0);
				set_target_position(target_x, target_y , 0, 0);
                #define POSITION_DEADBAND 5  // 死区值，单位取决于编码器的单位
                set_target_position(target_x, target_y , 0, 0);
        ...
        set_target_position(-95, 150, 0, 0);
        right = 0;
        left = 0;
        up = 1;
        down = 0;
        ...
        set_target_position(-95, 230, 0, 0);
        right = 0;
        left = 1;
        up = 0;
        down = 0;
		...
		while( pi_command != 9991 || pi_command != 9992 || pi_command != 9993 || pi_command != 111){


			if(x_move<=1000&&y_move<=1000 ){
				#define POSITION_DEADBAND 1  // 死区值，单位取决于编码器的单位
				mini_pos_corr = 1;
                sign_bazhi = 1;
				InitPIDControllers(); 
				...
				if(pi_command == 9992){
					...
					while( catch_fir_flag < 3){
                        ...
                        pick_car_out_red();watch_Initial_Position2();
                        target_yaw = 47 ;
                        catch_fir_flag++;
                        pick_car_out_blue();watch_Initial_Position2();
                        target_yaw = 47 ;
                        catch_fir_flag++;
                        pick_car_out_green();watch_Initial_Position2();
                        target_yaw = 47 ;
                        catch_fir_flag++;
                        pi_command = 111;
                        break;
        ...
        watch_Initial_Position3();   //侧面视觉动作抬伸
        char test_message3[] = "3\n";
        for(int i = 0;i<10;i++){
        UART5_Transmit((uint8_t *)test_message2, strlen(test_message2));
		...
		while( pi_command != 9991 || pi_command != 9992 || pi_command != 9993){
			...
			if(x_move<=1000&&y_move<=1000 ){
				...
				if(pi_command == 9992){
					...
					while( catch_fir_flag < 2){

                        pick_car_out_red_cemian();watch_Initial_Position3();
                        target_yaw = 47 ;
                        catch_fir_flag++;
                        pick_car_out_blue_cemian();watch_Initial_Position3();
                        target_yaw = 47 ;
                        catch_fir_flag++;
                        pick_car_out_green_cemian();watch_Initial_Position3();
                        target_yaw = 47 ;
                        catch_fir_flag++;
                        pi_command = 0;
